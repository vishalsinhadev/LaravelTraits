...
trait SearchableTrait
{
    ...
    public function scopeSearchRestricted(Builder $q, $search, $restriction, $threshold = null, $entireText = false, $entireTextOnly = false)
    {
        ...
        $words = array_prepend($words, $search);
        ...
        foreach ($this->getColumns() as $column => $relevance) {
            ...
            if (($entireText === true && count($words) > 1) || $entireTextOnly === true) {
                $queries[] = $this->getSearchQuery($query, $column, $relevance, [
                    $search
                ], 50, '', '');
                $queries[] = $this->getSearchQuery($query, $column, $relevance, [
                    $search
        ...
        $jsonCompare = addslashes(json_encode([
            $search
        ]));

        foreach ($this->getJsonColumns() as $column => $relevance) {
            $relevance_count += $relevance;
            $column = str_replace('.', '`.`', $column);
            $field = 'JSON_CONTAINS(LOWER(`' . $column . '`), "' . $jsonCompare . '")';
            $selects[] = '(case when ' . $field . ' then ' . (50 * $relevance) . ' else 0 end)';
        }
     ...
     * @return array
     */
    protected function getJsonColumns()
    {
        if (array_key_exists('json_columns', $this->searchable)) {
            $driver = $this->getDatabaseDriver();
            $prefix = Config::get("database.connections.$driver.prefix");
            $columns = [];
            foreach ($this->searchable['json_columns'] as $column => $priority) {
                $columns[$prefix . $column] = $priority;
            }
            return $columns;
        } else {
